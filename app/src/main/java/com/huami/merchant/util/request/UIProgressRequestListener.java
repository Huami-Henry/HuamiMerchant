...
public abstract class UIProgressRequestListener implements ProgressRequestListener {
            ...
            switch (msg.what) {
                case REQUEST_UPDATE:
                    ...
                    if (uiProgressRequestListener != null) {
                        ...
                        //回调抽象方法
                        uiProgressRequestListener.onUIRequestProgress(progressModel.getTag(),progressModel.getCurrentBytes(), progressModel.getContentLength(), progressModel.isDone());
                    }
                    break;
                default:
    ...
    @Override
    public void onRequestProgress(String tag,long bytesRead, long contentLength, boolean done) {
        //通过Handler发送进度消息
        Message message = Message.obtain();
        message.obj = new ProgressModel(tag,bytesRead, contentLength, done);
     ...
     * @param done 是否写入完成
     */
    public abstract void onUIRequestProgress(String tag,long bytesWrite, long contentLength, boolean done);
}